...
function answer($string,$mysqli,$fd){
    ...
    exit(); 
     ...
     if ($stmt = $mysqli->prepare("SELECT id,input FROM friender WHERE 1")) { 
        $stmt->execute(); 
       ...
       try {
           if(!$string==null){
            $stmt = $mysqli->prepare("INSERT INTO friender VALUES (0, ?, '0', 0)"); 
...
function get($idex){
        ...
        if ($stmt1 = $mysqli1->prepare("SELECT output FROM friender WHERE id={$idex}")) {
        $stmt1->execute(); 
    ...
    if(!$ans==0){
        $stmt2 = $mysqli1->prepare("UPDATE friender SET `freq`=`freq`+1 WHERE id='{$idex}'"); 
        $stmt2->execute(); 
        $stmt2->close();
